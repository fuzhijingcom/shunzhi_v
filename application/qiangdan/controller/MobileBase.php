...
class MobileBase extends Controller {
    ...
    public function _initialize() {
        ...
        $qianguser  = M('users_qiang')->where('user_id',$user_id)->find();
        $school = $qianguser['school'];
        if(!empty($qianguser) && $school == NULL ){
            
            $url  = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
            $url = urlencode($url);
            $this->redirect('my/user/school_qiang', array('url' => $url), 1, '请先选择学校...');
        }
